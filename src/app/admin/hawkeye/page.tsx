  ...
  return (
    <div className="max-w-4xl">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-3 mb-6">
        <Card className={cancelCount > 0 ? 'border-red-200 bg-red-50/50' : ''}>
        ...
        <Card className={reprintCount > 0 ? 'border-amber-200 bg-amber-50/50' : ''}>
          <CardContent className="p-4">
            <div className="flex items-center gap-2 mb-1">
              <Printer className="h-4 w-4 text-amber-600" />
              <p className="text-xs text-gray-500">Reprinted</p>
            </div>
            <p className="text-2xl font-bold text-amber-600">{reprintCount}</p>
          </CardContent>
        </Card>
        <Card className={refundCount > 0 ? 'border-orange-200 bg-orange-50/50' : ''}>
          <CardContent className="p-4">
            <div className="flex items-center gap-2 mb-1">
        ...
        <Card className={partialCount > 0 ? 'border-blue-200 bg-blue-50/50' : ''}>
          <CardContent className="p-4">
            <div className="flex items-center gap-2 mb-1">
              <Clock className="h-4 w-4 text-blue-500" />
              <p className="text-xs text-gray-500">Partial/Balance</p>
            </div>
      ...
      <div className="flex gap-1 bg-gray-100 rounded-lg p-1 mb-4 overflow-x-auto">
        ...
        <button
          ...
          className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md text-sm font-medium transition-colors whitespace-nowrap ${
            filter === 'item_cancel' ? 'bg-white shadow text-red-700' : 'text-gray-600'
          }`}
        >
        ...
        <button
          ...
          className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md text-sm font-medium transition-colors whitespace-nowrap ${
            filter === 'bill_reprint' ? 'bg-white shadow text-amber-700' : 'text-gray-600'
          }`}
        >
        ...
        <button
          ...
          className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md text-sm font-medium transition-colors whitespace-nowrap ${
            filter === 'refund' ? 'bg-white shadow text-orange-700' : 'text-gray-600'
          }`}
        >
            ...
            <div className="divide-y">
              {filteredLogs.map((log) => {
                const actionConfig: Record<AuditAction, { bg: string; text: string; icon: typeof Trash2; label: string }> = {
                  item_cancel: { bg: 'bg-red-100', text: 'text-red-600', icon: Trash2, label: 'Item Cancelled' },
                  bill_reprint: { bg: 'bg-amber-100', text: 'text-amber-600', icon: Printer, label: 'Bill Reprinted' },
                  refund: { bg: 'bg-orange-100', text: 'text-orange-600', icon: RotateCcw, label: 'Refund' },
                  partial_payment: { bg: 'bg-blue-100', text: 'text-blue-600', icon: Clock, label: 'Partial Payment' },
                  balance_collected: { bg: 'bg-green-100', text: 'text-green-600', icon: Wallet, label: 'Balance Collected' },
                  daily_closing: { bg: 'bg-gray-100', text: 'text-gray-600', icon: CalendarCheck, label: 'Day Closed' },
                }
                        ...
                        {log.action === 'item_cancel' && (
                          <>
                            <p className="text-sm font-medium">
                              ...
                              <span className="text-gray-500 font-normal"> — {log.details.order_number}</span>
                              {log.details.table && (
                                <span className="text-gray-400 font-normal"> ({log.details.table})</span>
                              )}
                            </p>
                            {log.details.reason && (
                        ...
                        {log.action === 'daily_closing' && (
                          <p className="text-sm font-medium">
                            ...
                            {log.details.total_sales != null && (
                              <span className="text-gray-500 font-normal"> — Sales ₹{Number(log.details.total_sales).toFixed(0)}, {log.details.total_orders} orders</span>
                            )}
                          </p>
                        )}

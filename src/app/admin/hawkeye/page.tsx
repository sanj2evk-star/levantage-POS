...
export default function HawkEyePage() {
  ...
  return (
    <div className="max-w-4xl">
        ...
        <Card className={reprintCount > 0 ? 'border-amber-200 dark:border-amber-800 bg-amber-50/50 dark:bg-amber-900/20' : ''}>
          <CardContent className="p-4">
            <div className="flex items-center gap-2 mb-1">
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400">Reprinted</p>
            </div>
        ...
        <Card className={partialCount > 0 ? 'border-blue-200 dark:border-blue-800 bg-blue-50/50 dark:bg-blue-900/20' : ''}>
          <CardContent className="p-4">
            <div className="flex items-center gap-2 mb-1">
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400">Partial/Balance</p>
            </div>
      ...
      {/* Filter tabs */}
      <div className="flex gap-1 bg-gray-100 dark:bg-gray-800 rounded-lg p-1 mb-4 overflow-x-auto">
        <button
          onClick={() => setFilter('all')}
          className={`px-3 py-1.5 rounded-md text-sm font-medium transition-colors whitespace-nowrap ${
            filter === 'all' ? 'bg-white dark:bg-gray-900 shadow text-gray-900 dark:text-gray-100' : 'text-gray-600 dark:text-gray-400'
          }`}
        >
          All ({logs.length})
        </button>
        <button
          ...
          className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md text-sm font-medium transition-colors whitespace-nowrap ${
            filter === 'item_cancel' ? 'bg-white dark:bg-gray-900 shadow text-red-700' : 'text-gray-600 dark:text-gray-400'
          }`}
        >
        ...
        <button
          ...
          className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md text-sm font-medium transition-colors whitespace-nowrap ${
            filter === 'bill_reprint' ? 'bg-white dark:bg-gray-900 shadow text-amber-700' : 'text-gray-600 dark:text-gray-400'
          }`}
        >
        ...
        <button
          ...
          className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md text-sm font-medium transition-colors whitespace-nowrap ${
            filter === 'refund' ? 'bg-white dark:bg-gray-900 shadow text-orange-700' : 'text-gray-600 dark:text-gray-400'
          }`}
        >
            ...
            <div className="divide-y dark:divide-gray-700">
              {filteredLogs.map((log) => {
                const actionConfig: Record<AuditAction, { bg: string; text: string; icon: typeof Trash2; label: string }> = {
                  ...
                  daily_closing: { bg: 'bg-gray-100 dark:bg-gray-800', text: 'text-gray-600 dark:text-gray-400', icon: CalendarCheck, label: 'Day Closed' },
                }
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          <span className="text-xs text-gray-400 dark:text-gray-500">{fmtTime(log.created_at)}</span>
                        </div>
                        ...
                        {log.action === 'item_cancel' && (
                          <>
                            <p className="text-sm font-medium">
                              ...
                              <span className="text-gray-500 dark:text-gray-400 font-normal"> — {log.details.order_number}</span>
                              {log.details.table && (
                                <span className="text-gray-400 dark:text-gray-500 font-normal"> ({log.details.table})</span>
                              )}
                            </p>
                            {log.details.reason && (
                        ...
                        {log.action === 'daily_closing' && (
                          <p className="text-sm font-medium">
                            ...
                            {log.details.total_sales != null && (
                              <span className="text-gray-500 dark:text-gray-400 font-normal"> — Sales ₹{Number(log.details.total_sales).toFixed(0)}, {log.details.total_orders} orders</span>
                            )}
                          </p>
                        )}

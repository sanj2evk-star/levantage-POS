                  ...
                  {existingPayments.map(p => (
                    <div key={p.id} className="flex justify-between text-sm">
                      <span className="flex items-center gap-2">
                        ...
                        {p.reference_number && <span className="text-xs text-gray-400 dark:text-neutral-500">Ref: {p.reference_number}</span>}
                      </span>
          ...
          <div className="px-8 py-3 bg-gradient-to-r from-amber-50 to-orange-50 dark:from-amber-950/40 dark:to-orange-950/40 border-b border-amber-100 dark:border-amber-900 shrink-0">
            <div className="flex items-center justify-between pr-8">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h2 className="text-lg font-bold text-gray-900 dark:text-neutral-100 tracking-tight">{tableName}</h2>
                  <div className="flex items-center gap-2 mt-0.5">
                    <span className="text-xs text-gray-500 dark:text-neutral-400 font-mono">{order.order_number}</span>
            ...
            <div className="w-[40%] border-r border-gray-200 dark:border-neutral-600 overflow-y-auto bg-white dark:bg-neutral-800">
              <div className="px-6 py-4 space-y-3">
                ...
                <div className="flex items-center justify-between">
                  <p className="text-xs font-semibold text-gray-400 dark:text-neutral-500 uppercase tracking-wider">Order Items</p>
                  <span className="text-xs text-gray-400 dark:text-neutral-500 font-medium">{activeItems.length} items</span>
                </div>
                ...
                <div className="space-y-0.5">
                    ...
                    <div key={item.id} className="flex items-center justify-between py-2 px-3 rounded-lg hover:bg-gray-50 dark:hover:bg-neutral-600 transition-colors group">
                      <div className="flex items-center gap-3 min-w-0 flex-1">
                        ...
                        <div className="min-w-0 flex-1">
                          <p className="text-sm font-medium text-gray-800 dark:text-neutral-200">{item.menu_item?.name}</p>
                          <p className="text-[11px] text-gray-400 dark:text-neutral-500">{item.quantity} × ₹{item.unit_price.toFixed(0)}</p>
                        </div>
                      </div>
                      <div className="flex items-center gap-2 shrink-0 ml-2">
                        <span className="text-sm font-semibold text-gray-700 dark:text-neutral-300 tabular-nums">₹{item.total_price.toFixed(0)}</span>
                        {order.status !== 'completed' && !(order.bill_print_count && order.bill_print_count > 0) && (
                          <button onClick={() => openCancelDialog(item.id, item.menu_item?.name || 'Item')}
                            className="h-6 w-6 flex items-center justify-center rounded-md text-gray-300 dark:text-neutral-600 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/30 transition-colors opacity-0 group-hover:opacity-100">
                  ...
                  {localItems.filter(i => i.is_cancelled).map(item => (
                    <div key={item.id} className="flex justify-between py-1 px-3 text-xs text-gray-400 dark:text-neutral-500 line-through">
                ...
                <div className="space-y-2 px-1">
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="font-medium text-gray-700 dark:text-neutral-300 tabular-nums">₹{subtotal.toFixed(2)}</span>
                  </div>
                  <div className="flex justify-between text-sm items-center">
                    <span className="flex items-center gap-2 text-gray-500 dark:text-neutral-400">
                    ...
                    <span className={`tabular-nums font-medium ${serviceChargeRemoved ? 'line-through text-gray-300 dark:text-neutral-600' : 'text-gray-700 dark:text-neutral-300'}`}>
                  ...
                  <div className="flex justify-between text-sm">
                    <span className="text-gray-500 dark:text-neutral-400">GST ({GST_PERCENT}%)</span>
                    <span className="font-medium text-gray-700 dark:text-neutral-300 tabular-nums">₹{gstAmount.toFixed(2)}</span>
                  </div>
                </div>
                ...
                <div className="bg-gradient-to-r from-amber-50 to-orange-50 dark:from-amber-950/40 dark:to-orange-950/40 rounded-xl px-4 py-2.5 border border-amber-200 dark:border-amber-800">
                  <div className="flex justify-between items-center">
                    <span className="font-bold text-gray-800 dark:text-neutral-200">Grand Total</span>
                          ...
                          <div key={index} className="bg-white dark:bg-neutral-800 rounded-xl p-3 border border-violet-100 dark:border-violet-900/50 space-y-2.5 shadow-sm">
                            <div className="flex items-center justify-between">
                              <div className="flex items-center gap-2">
                                ...
                                <span className="text-sm font-medium text-gray-700 dark:text-neutral-300">Payment {index + 1}</span>
                              </div>
            ...
            {(waiters || []).filter(w => w.id !== order.waiter_id).map(waiter => (
              ...
              >
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-400 dark:text-neutral-500 capitalize">{waiter.role}</p>
                </div>
              </button>
            ))}
            {(waiters || []).filter(w => w.id !== order.waiter_id).length === 0 && (
              <p className="text-center text-gray-400 dark:text-neutral-500 py-4">No other staff available</p>
            )}
          </div>
        </DialogContent>
      </Dialog>
